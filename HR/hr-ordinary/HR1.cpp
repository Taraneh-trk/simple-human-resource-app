...
class DatabaseTeam : public OrdinaryStaff
{
	public:
		...
		setDatabaseTeammember(personinformation personionfo)
		{
			OrdinaryStaff::setOrdinaryStaff(personionfo);
		}
};
class ProgrammingTeam : public OrdinaryStaff
{
	public:
		 ...
		 setProgrammingTeammember(personinformation personionfo)
		 {
		 	OrdinaryStaff::setOrdinaryStaff(personionfo);
		 }
...
class Tester : public OrdinaryStaff
{
	public:
		...
		setTestermember(personinformation personionfo)
		{
			OrdinaryStaff::setOrdinaryStaff(personionfo);
		}
};
	...
	public:
		...
		long long int teamnum(string position)
		{
			...
			while(!finfo.eof())
			{
				...
				getline(finfo,strtemp);
				prsn.workexperience = atoi(strtemp.c_str());
		...
		void show(t team,int teampos)
		{
			...
			while(!finfo.eof())
			{
				cout<<'\n';
				cout<<"\nmember number "<<i<<" : ";
				getline(finfo,team.personionfo.name);
				getline(finfo,team.personionfo.lastname);
				getline(finfo,strtemp);
				team.personionfo.level = getnum(atoi(strtemp.c_str()),0);
				getline(finfo,strtemp);
				team.personionfo.teamlevel = getnum(atoi(strtemp.c_str()));
				getline(finfo,strtemp);
				team.personionfo.salary = atoi(strtemp.c_str());
				getline(finfo,strtemp);
				team.personionfo.workexperience = atoi(strtemp.c_str());
			...
			while(1)
			{
				...
				if(flagsearchperson)
				{
					...
					for(int j=0;j<membernum;j++)
					{
						...
						{
							string strtemp;
							getline(finfo,prsns[i].personionfo.name);
							getline(finfo,prsns[i].personionfo.lastname);
							getline(finfo,strtemp);
							prsns[i].personionfo.level = getnum(atoi(strtemp.c_str()),0);
							getline(finfo,strtemp);
							prsns[i].personionfo.teamlevel = getnum(atoi(strtemp.c_str()));
							getline(finfo,strtemp);
							prsns[i].personionfo.salary = atoi(strtemp.c_str());
							getline(finfo,strtemp);
							prsns[i].personionfo.workexperience = atoi(strtemp.c_str());
							i++;
						}else{
							string strtemp;
							getline(finfo,prsntemp.personionfo.name);
							getline(finfo,prsntemp.personionfo.lastname);
							getline(finfo,strtemp);
							prsns[i].personionfo.level = getnum(atoi(strtemp.c_str()),0);
							getline(finfo,strtemp);
							prsns[i].personionfo.teamlevel = getnum(atoi(strtemp.c_str()));
							getline(finfo,strtemp);
							prsntemp.personionfo.salary = atoi(strtemp.c_str());
							getline(finfo,strtemp);
							prsntemp.personionfo.workexperience = atoi(strtemp.c_str());
						}
						num++;
					}
					finfo.close();
		...
		void search(t team,int teampos,int whole = 0)
		{
			...
			if(flagsearchperson)
			{
				...
				if(whole == 0 || whole == 2)     //<<position<<'\n'
				{
					cout<<"\nsuch person was not found . \nplease try again . \n";
					flagsearchperson = false;
					prsn1.name = "not found";
					prsn1.lastname = "not found";
				}
			}
			finfo.close();
		}
		void edit()
		{
			while(1)
			{
				...
				if(flagsearchperson)
				{
					...
					for(int j=0;j<membernum;j++)
					{
						if(num != prsn1.membernuminfile)   //  i    j    num    ?????
						{
							string strtemp;
							getline(finfo,prsns[j].personionfo.name);
							getline(finfo,prsns[j].personionfo.lastname);
							getline(finfo,strtemp);
							prsns[j].personionfo.level = getnum(atoi(strtemp.c_str()),0);
							getline(finfo,strtemp);
							prsns[j].personionfo.teamlevel = getnum(atoi(strtemp.c_str()));
							getline(finfo,strtemp);
							prsns[j].personionfo.salary = atoi(strtemp.c_str());
							getline(finfo,strtemp);
							prsns[j].personionfo.workexperience = atoi(strtemp.c_str());
						}else{
							string strtemp;
							getline(finfo,prsns[j].personionfo.name);
							getline(finfo,prsns[j].personionfo.lastname);
